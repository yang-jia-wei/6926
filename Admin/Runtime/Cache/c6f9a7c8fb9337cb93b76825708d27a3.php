...
<?php require APP_ROOT.'public/top2.php';?>
 ...
 $where = array(); $where['type_id'] = $type_id; $where['list_switch'] = 2; $list_input=M('input')->where($where)->order('date asc')->select(); foreach($list_input as $k=>$v){ ?>
				...
				<?php
 $perpage=20; $offset=($p-1)*$perpage; $where = array(); $where['r.classify_id'] = $classify_id; $where['r.type_id'] = $type_id; $list = M($table_name)->where(array('goods_id' => $content_id))->order('date desc')->limit($offset,$perpage)->select(); $total_num = M($table_name)->where(array('goods_id' => $content_id))->order('date desc')->count(); foreach($list as $k=>$v){ $v['content_id']=$v[$table_name.'_id']; ?>
					<tr>
						<td height="35">
