...
<div class="content">
  ...
  <?php $member_id=pg('member_id'); $type_id=51; $data=pg('data'); $search_type=pg('search_type'); $search=pg('search'); $lt_date=pg('lt_date'); $gt_date=pg('gt_date'); if($search_type=='member_id') { $data[$search_type]=$search; } else if($search!='') { $member=M('member')->where(array($search_type=>$search))->field('member_id')->find(); $data['member_id']=array('in',$member); } if($member_id!='') { $where['member_id']=$member_id; $search=$member_id; } if($lt_date && $gt_date) { $where['date']=array(array('gt',strtotime($gt_date)),array('lt',strtotime($lt_date))); } else if($lt_date) { $where['date']=array('lt',strtotime($lt_date)); } else if($gt_date) { $where['date']=array('gt',strtotime($gt_date)); } foreach($data as $k=>$v) { if($v)$where[$k]=$v; } ?>
...
<form action="admin.php?m=member&a=account" method="post">
    ...
    <table class="tables" width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <?php
 $perpage=20; $offset=($p-1)*$perpage; $account=M('account')->where($where)->limit($offset,$perpage)->order('account_id desc')->select(); $total_num=M('account')->where($where)->count(); foreach($account as $key=>$v){ ?>
      <tr>
        ...
        <td><?php
 if($v['amount_type']==1) { echo "+".$v['amount']; } else { echo "-".$v['amount']; }?></td>
