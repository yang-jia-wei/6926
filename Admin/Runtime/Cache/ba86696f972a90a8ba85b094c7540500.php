...
<div class="content">
	...
	<?php
 $type_id = pg('type_id'); $classify_id = pg('classify_id'); $content_id = pg('content_id'); $goods_id = pg('goods_id'); $table_name = M('classify_type')->where(array('type_id' => $type_id))->getField('table_name'); ?>
	<div class="right_panel" id="right"  style="padding-left:0;">
<?php require APP_ROOT.'public/top2.php';?>
     <div class="right_data" id="sites">
		<a href="admin.php?m=content&a=biaoqian&type_id=70&classify_id=<?php echo $classify_id; ?>&content_id=<?php echo $goods_id; ?>" class="button">返回列表</a>
		<a href="javascript:;" onclick="dialog_div(900,600,'admin.php?m=content&a=add&type_id=<?php echo $type_id;?>&classify_id=<?php echo $classify_id;?>&ajax=1&time=<?php echo time()?>&biaoqian_id=<?php echo $content_id;?>')" class="button">添加内容</a>
 ...
 $where = array(); $where['type_id'] = $type_id; $where['list_switch'] = 2; $list_input=M('input')->where($where)->order('date asc')->select(); foreach($list_input as $k=>$v){ ?>
				...
				<?php
 $perpage=20; $offset=($p-1)*$perpage; $where = array(); $where['r.classify_id'] = $classify_id; $where['r.type_id'] = $type_id; $list = M($table_name)->where(array('biaoqian_id' => $content_id))->order('date desc')->limit($offset,$perpage)->select(); $total_num = M($table_name)->where(array('biaoqian_id' => $content_id))->order('date desc')->count(); foreach($list as $k=>$v){ $v['content_id']=$v[$table_name.'_id']; ?>
					<tr>
						<td height="35">
