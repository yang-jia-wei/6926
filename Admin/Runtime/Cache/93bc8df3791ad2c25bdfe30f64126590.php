...
<!--                    <td>审核</td>-->
				...
				<?php
 $perpage=20; $offset=($p-1)*$perpage; $list = M($table_name)->limit($offset,$perpage)->order('date desc')->select(); $total_num = M($table_name)->count(); if($search){ $sear['member_id']=array('like',"%$search%"); $sear['username']=array('like',"%$search%"); $sear['_logic']='OR'; $list=M('member')->where($sear)->select(); $total_num=10; } foreach($list as $k=>$v){ ?>
					<tr>
