...
<div class="codehtml"></div>
      <table class="tables" id="sites" width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr class="title">
  ...
  <tr class="row">
  <td class="form_tit">&nbsp;&nbsp;<?php echo $v['input_name']?></td>
  <td>
    <?php  switch($v['input_type_id']){ case 1: if($v['input_id']==453 && $user['user_id']>1){ echo $list[$v['field_name']].'条'; }else{ ?>
        <input name="data[<?php echo $v['field_name'];?>]" class="type_1" type="text" id="<?php echo $v['field_name'];?>" value="<?php echo $list[$v['field_name']];?>" size="80" />
        <?php
 } break; case 2: ?>
...
<?php }?>
        ...
        <?php } }?>
        <tr>
          <td>&nbsp;&nbsp;<input style="margin-left:26px;" name="" type="submit" class="submit" value="确认无误，提交" /></td>
        </tr>
      </table>
    </form>
   </div> 
